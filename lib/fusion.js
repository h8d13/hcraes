function normalizeUrl(u) {
  try {
    const url = new URL(u);
    // Only http(s) survives. Drops javascript:/data:/etc. so a hostile
    // source result can't reach a clickable href. Non-parseable urls -> null.
...
export function rrf(perSource, sources, k = 60) {
  ...
  for (const [name, results] of perSource) {
    ...
    for (const r of results) {
      ...
      if (!key) continue; // non-http(s) or unparseable url, skip entirely
